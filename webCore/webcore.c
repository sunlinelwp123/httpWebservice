...
void *http_handle(int client){
	...
	//代码已遗弃
		...
		_buf[1] = '\0';
		printf("POST \n");
		//POST请求先找 内容长度 content_length 
		_line = read_line(client, _buf ,sizeof(_buf));
		while((_line > 0)){
			buf[15] = '\0';
			printf("buf:%s\n", _buf);
			if(strcasecmp(_buf, "Content-Length:") == 0){
				content_length = atoi(&(buf[16]));
				printf("content_length%d\n", content_length);
			}
			#if 0
			if(strcasecmp(_buf, "Content-Type:") == 0){
			//	strcat(content_type ,buf[16]);
				printf("content_type%d\n", content_type);
			}
			#endif
			_line = read_line(client, _buf,sizeof(_buf));
		}
		printf("while end length%d\n", content_length);
		if(content_length == -1){
			//请求错误处理
			//暂时用404
			not_found(client);	
